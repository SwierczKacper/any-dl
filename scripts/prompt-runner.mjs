...
import { confirm, input, select } from '../src/prompt.js';
...
if (mode === 'confirm-default-yes') {
	...
	result = await select({ message: 'Pick one', choices: CHOICES });
} else if (mode === 'select-actions') {
	result = await select({
		message: 'Pick one',
		choices: CHOICES,
		actions: [{ key: 'c', hint: 'c other list', value: 'SWITCHED' }],
	});
} else {
